...
project_name = "Parliament Sentiment Analysis Dashboard"
...
def load_model():
    ...
    scrappedReviews = pd.read_csv('chat_log.csv')
...
def check_review(textText):
    transformer = TfidfTransformer()
    loaded_vec = CountVectorizer(decode_error="replace", vocabulary=vocab)
    reviewText = transformer.fit_transform(loaded_vec.fit_transform([textText]))
    return pickle_model.predict(textText)
            ...
            [
                ...
                dbc.Container([
                    dcc.Dropdown(
                        ...
                        options=[{'label': i[:60] + "...", 'value': i} for i in scrappedReviews.text[:1000]],
                        value=scrappedReviews.text[0],
                        style={'margin-bottom': '30px'}
                ...
                html.Div(id='result1'),
                dbc.Textarea(id='textarea', className="mb-3", placeholder="Enter the Review", value='',
                             style={'height': '150px'}),
